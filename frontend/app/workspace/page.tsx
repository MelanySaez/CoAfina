...
export default function Home() {
    ...
    const fetchCountries = async () => {
      try {
        ...
        const response = await fetch(
          buildApiUrl(API_CONFIG.endpoints.countries)
        );
        if (!response.ok) {
        ...
        const transformedCountries: CountryPlacement[] = data.countries.map(
        ...
        setError(
          "No se pudieron cargar los países. Verifica que el backend esté ejecutándose."
        );
      } finally {
        setLoading(false);
      }
    ...
    const fetchMetrics = async () => {
      ...
      try {
        const metricsPromises = countries.map(async (country) => {
          try {
            const response = await fetch(
              buildApiUrl(API_CONFIG.endpoints.countryData(country.name))
            );
            ...
            const aee_pom_avg =
              data.data.reduce(
                (sum: number, record: any) => sum + record.AEE_POM_kg_hab,
                0
              ) / totalRecords;
            const raee_generados_avg =
              data.data.reduce(
                (sum: number, record: any) =>
                  sum + record.RAEE_generados_kg_hab,
                0
              ) / totalRecords;
  ...
  const calculateMetrics = (countriesSide: CountryPlacement[]) => {
    ...
    countriesSide.forEach((country) => {
      if (countryMetrics[country.name]) {
        ...
        metrics.raee_generados +=
          countryMetrics[country.name].raee_generados_avg;
      }
    });
    return metrics;
  };
  ...
  const handleDropOnSide = (side: "left" | "right") => {
    ...
    setCountries(
      countries.map((c) => (c.id === draggedCountry ? { ...c, side } : c))
    );
  ...
  const handleDropOnUnplaced = () => {
    if (!draggedCountry) return;
    setCountries(
      countries.map((c) => (c.id === draggedCountry ? { ...c, side: null } : c))
    );
    setDraggedCountry(null);
  };
              ...
              {loading && (
                <div className="bg-card rounded-lg border border-border p-8 text-center">
                  ...
                  <p className="text-sm text-muted-foreground">
                    Cargando países desde el backend...
                  </p>
                </div>
              )}
              ...
              {error && (
                <div className="bg-destructive/10 border border-destructive/50 rounded-lg p-4">
                  ...
                  <p className="text-xs text-muted-foreground mt-1">
                    Espere un momento que el backend se inicie
                  </p>
                </div>
              )}
              ...
              {!loading && !error && (
                <>
                    ...
                    <div className="flex items-center justify-between mb-4">
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground mt-1 font-medium">
                          Arrastra los países a los lados para compararlos (
                          {unplacedCountries.length} disponibles)
                        </p>
                      </div>
                      {(leftCountries.length > 0 ||
                  ...
                  <div className="flex gap-6">
                      ...
                      <div className="flex-1 grid grid-cols-5 gap-2 mb-6">
                        {leftCountries.map((country) => (
                          ...
                          >
                            ...
                            <div className="text-[9px] font-bold text-foreground leading-tight truncate w-full px-0.5">
                              {country.name}
                            </div>
                          </div>
                        ))}
                      ...
                      <div className="border-t-2 border-primary/30 pt-5 space-y-3 bg-gradient-to-r from-primary/5 to-transparent rounded-lg p-4">
                        <div className="flex items-center justify-between">
                          ...
                          <span className="text-lg font-black text-primary">
                            {leftMetrics.aee_pom.toFixed(2)} kg/hab
                          </span>
                        </div>
                        <div className="flex items-center justify-between">
                          <span className="text-sm font-bold text-muted-foreground">
                          ...
                          <span className="text-lg font-black text-primary">
                            {leftMetrics.raee_generados.toFixed(2)} kg/hab
                          </span>
                        </div>
                      </div>
                    </div>
                    ...
                    >
                      <div className="mb-6">
                        <h3 className="text-xl font-black text-secondary mb-1">
                          Análisis B
                        </h3>
                      ...
                      <div className="border-t-2 border-secondary/30 pt-5 space-y-3 bg-gradient-to-r from-secondary/5 to-transparent rounded-lg p-4">
                        <div className="flex items-center justify-between">
                          ...
                          <span className="text-lg font-black text-secondary">
                            {rightMetrics.aee_pom.toFixed(2)} kg/hab
                          </span>
                        </div>
                        <div className="flex items-center justify-between">
                          <span className="text-sm font-bold text-muted-foreground">
                          ...
                          <span className="text-lg font-black text-secondary">
                            {rightMetrics.raee_generados.toFixed(2)} kg/hab
                          </span>
                        </div>
                      </div>
                    </div>

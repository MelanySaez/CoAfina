...
import { Upload, X, Search, Plus, FileText, TrendingUp, RefreshCw } from "lucide-react"
...
export function DataPanelTab() {
  ...
  const combinedData = (() => {
    ...
    const result = sortedYears.map(year => {
      ...
      selectedFiles.forEach(file => {
        ...
        file.parsedCSV.forEach(row => {
          const rowYear = Number(row['Año'] || row['año'] || row['Year'] || row['Ano'])
          if (rowYear === year) {
            // Si hay país en el nombre del archivo, usarlo; si no, buscar en el CSV
            const country = countryFromFile || 
                           (row['Pais'] || row['País'] || row['Country'] || row['pais'] || '').trim()
            
            const value1 = Number(row['AEE_POM_kg_hab'] || row['AEE POM kg/hab'] || 0)
            const value2 = Number(row['RAEE_generados_kg_hab'] || row['RAEE generados kg/hab'] || 0)
  ...
  const colors = [
    "#3b82f6", "#8b5cf6", "#f59e0b", "#ec4899", "#06b6d4", 
    "#10b981", "#f97316", "#6366f1", "#14b8a6", "#eab308"
  ]
                ...
                return (
                  <div
                    ...
                    className={`flex items-center gap-3 p-3 rounded-xl border-2 transition-all duration-300 group cursor-pointer ${
                      file.selected
                        ? `bg-gradient-to-r ${fileColors.bg} ${fileColors.border} shadow-lg shadow-blue-500/20`
                  ...
                  >
                    ...
                    />
                    <FileText
                      className={`h-4 w-4 transition-colors ${file.selected ? fileColors.text : "text-muted-foreground"}`}
                    />
                    <span
                      className={`flex-1 text-sm font-medium truncate transition-colors ${
                        file.selected ? `font-semibold ${fileColors.text}` : "text-muted-foreground"
                      }`}
                    >
                      {file.name}
                    </span>
            ...
            <div className="space-y-6">
              ...
              <div className="bg-gradient-to-br from-blue-50/30 to-blue-50/10 dark:from-blue-950/20 dark:to-transparent rounded-2xl border-2 border-blue-200/30 dark:border-blue-800/30 p-6 shadow-xl hover:shadow-2xl hover:shadow-blue-500/10 transition-all duration-300">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <h4 className="text-base font-bold text-foreground">Tendencia por Año</h4>
                </div>
              ...
              <div className="bg-gradient-to-br from-emerald-50/30 to-emerald-50/10 dark:from-emerald-950/20 dark:to-transparent rounded-2xl border-2 border-emerald-200/30 dark:border-emerald-800/30 p-6 shadow-xl hover:shadow-2xl hover:shadow-emerald-500/10 transition-all duration-300">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <h4 className="text-base font-bold text-foreground">Comparación por Año (Barras)</h4>
                </div>
                <ResponsiveContainer width="100%" height={300}>
